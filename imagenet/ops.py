...
class batch_norm(object):
    ...
    """
    def __init__(self, batch_size, epsilon=1e-5, name="batch_norm", half=None):
...
class batch_norm_second_half(batch_norm):
...
class batch_norm_first_half(batch_norm):
...
class batch_norm_cross(batch_norm):
    def __init__(self, epsilon=1e-5,  name="batch_norm"):
        with tf.variable_scope(name) as scope:
            self.epsilon = epsilon
            self.name=name
...
def special_deconv2d(input_, output_shape,
    ...
    return deconv2d(input_, output_shape,
                    k_h=k_h, k_w=k_w, d_h=d_h, d_w=d_w,
                    stddev=stddev,name=name, with_w=wih_w,
                    init_bias=init_bias, special=True)
...
def offset_maxout(x, k = 2):
    return maxout(x, k=k, ofs=rng.randn(1000, k).max(axis=1).mean())
...
def decayer(x, name="decayer", shape=[1]):
    with tf.variable_scope(name):
        scale = tf.get_variable("scale", shape, initializer=tf.constant_initializer(1.))
        decay_scale = tf.get_variable("decay_scale", shape, initializer=tf.constant_initializer(1.))
        relu = tf.nn.relu(x)
